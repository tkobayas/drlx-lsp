# Phase 0: C3 Characterization Tests Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a test class that records antlr4-c3 candidate output at ~15 caret positions, establishing a baseline before any changes to PREFERRED_RULES.

**Architecture:** A single test class `DrlxC3CandidatesTest` with shared helpers (`collectAt`, `tokenNames`, `ruleCallStack`) and one `@Test` per caret position. Two fixture texts: a basic one for most positions, a separate one for inline-cast positions.
...
- Must run `mvn -pl drlx-completion -am install -DskipTests` before running tests (CLAUDE.md mandatory Maven rule)
- `PREFERRED_RULES` stays `{RULE_identifier}` — do not modify

---

### Task 1: Create DrlxC3CandidatesTest with helpers and all test methods

**Files:**
- Create: `drlx-completion/src/test/java/org/drools/drlx/completion/DrlxC3CandidatesTest.java`

**Interfaces:**
- Consumes: `DrlxParser`, `DrlxLexer`, `CodeCompletionCore`, `Tokens.IGNORED` (all existing)
- Produces: characterization tests (no downstream consumers)

- [ ] **Step 1: Create the test class with helpers and fixture texts**
...
class DrlxC3CandidatesTest {
    ...
    void constraintExpression() {
        ...
        // C3 reports what can follow: # (inline cast) and identifier (for constraint expr)
...
```
Expected: All tests pass including the new characterization tests

- [ ] **Step 4: Fix any assertion mismatches**

If any assertions fail, check the actual C3 output and update the assertion to match. The goal is to capture reality, not prescribe it.
...
git -C /home/tkobayas/usr/work/mvel3-development/drlx-lsp commit -m "test: add C3 characterization tests for completion positions

Record antlr4-c3 candidates.tokens, candidates.rules, and rule call
stacks at 15 caret positions. Establishes a baseline before any changes
to PREFERRED_RULES or completion context detection.
